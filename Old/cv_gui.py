#!/usr/bin/env python3
# One-Click CV Pipeline GUI (Free) — drop original CV .docx, then Process.
# Folders expected:
#   CV Script\Run_CV_GUI_ONECLICK.bat
#   CV Script\Processors\cv_gui_oneclick.py + your 4 processors
#   CV Script\Editable\  (for MASTER files)
# Outputs always go to: CV Script\Output\
...
try:
    ...
    try:
        ...
        raise SystemExit("FreeSimpleGUI not installed. Run via Run_CV_GUI_ONECLICK.bat to auto-install.")

HERE = Path(__file__).resolve().parent         # ...\CV Script\Processors
ROOT = HERE.parent                              # ...\CV Script
EDIT = ROOT / "Editable (Add CV here)"
OUT  = ROOT / "Output"
...
layout = [
    [sg.Text("Browse and Select the ORIGINAL CV (.docx) here, then press Process →", font=("Segoe UI", 11))],
    ...
    [sg.Frame("Log", [[sg.Multiline(size=(100,20), key="-LOG-", autoscroll=True, expand_x=True, expand_y=True, write_only=True)]], expand_x=True)]
]
window = sg.Window("CV Pipeline — One Click", layout, finalize=True)

def process_all(window, cv_path):
    ...
    window["-LOG-"].print("")
    window["-LOG-"].print(f"✓ Done. Output file:", text_color="green")
    window["-LOG-"].print(f" - {final_cv}")
...
while True:
    ...
    if ev == "Process":
        process_all(window, val["-CV-"].strip())
