#!/usr/bin/env python3
# Three-file CV Pipeline GUI (Free)
# Inputs:
#   1) Original CV (.docx)
...
#
# Outputs:
#   Only the final injected CV is written to: CV Script\Output\<same name as original>.docx

import os, sys, subprocess
from pathlib import Path

# Prefer license-free GUI
try:
...
layout = [
    [sg.Text("Select the three inputs, then press Process →", font=("Segoe UI", 11))],
...
window = sg.Window("CV Pipeline — Three Files", layout, finalize=True)

def process_all(window, cv_file: str, master_txt_file: str, master_red_file: str):
    ...
    # Clean intermediates so only the final file remains
...
while True:
    ...
    if ev == "Process":
        process_all(window, val["-CV-"], val["-MASTER_TXT-"], val["-MASTER_RED-"])
